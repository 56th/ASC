...
One standard approach to handle  multi-material cells is a homogenization procedure. In this approach, one assigns to the cells  artificial homogeneous  material properties, which are calculated based on the volumetric shares of different materials in each of the multi-material cells. This enables one to apply standard discretization {techniques}. %if one needs to solve for other physics, such as the heat transfer, on every time step of the simulation.
