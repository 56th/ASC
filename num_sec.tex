\section{Numerica results}\label{sec:num}
...
$$
        where $|\mcell|$ is the area of cell $\mcell$. While the disrete norm of the flux discretization error is defined as follows:
        $$
        \varepsilon_h^{l_2} =  \left[\sum\limits_{\bface \in \bmesh_h}
          ...
          \right]^\frac12
        $$
        where $A_\bface|$ is the representative are of a face $\bface$ which is equal to the arithmetic average
of areas of cells sharing the face $\bface$.
...
Numerical results are shown in Table~\ref{fig:conv:pwlin}. We observe that ASC(0) converges linearly with respect to the $\lTwo$-norm,
and ASC(1) has the convergence rate close to quadratic with respect to the $\lTwo$-norm.
The convergence of the flux errors is also is higher for ASC(1) method and is close to linear convergence.
