...
Before we go into details of the method, we note that $\lambda$ can also  be observed as the Lagrange multiplier corresponding to the normal flux condition~\eqref{flux_cond}. The problem \eqref{dual_mini}--\eqref{flux_cond} can be formulated in the weak form: \toDiscuss{Find}  $\vect u \in \bigotimes\limits_{\bcell\in\bmesh}\HDivNodeSpace[\bcell]$, $ p\in\LTwoSpace$, $\lambda \in \bigotimes\limits_{\bface\in\bfaces[int]}\HHalfSpace$ such that
...
\end{split}
\end{equation}
for all $\vect v \in \bigotimes\limits_{\bcell\in\bmesh}\HDivNodeSpace[\bcell]$, $q \in \LTwoSpace[\Omega]$, and $\mu \in \bigotimes\limits_{\bface\in\bfaces[int]}\HHalfSpace$\footnotemark{}.
